...
- [x] **Excel 导入菜单 + 每道菜个性化定制**：
  - 设置 → 「从 Excel 导入菜单」：按表头识别「分类 / 菜名 / 价格(可选)」，其余列**两两一组** `(定制项, 选项列表)`
  - 点菜时弹「**个性化定制**」：选定制项（Size / 辣度…）+ 填「**其他备注**」
  - 「其他备注」有**常用标签**可点选、可输入新的、可勾选**存成新标签**（标签在设置里管理）
  - 选项/备注会带进购物车、厨房单、顾客小票（**同菜不同选项 = 不同行**）
- [x] **UI 改进**：点单**两步**（先选种类 → 再选菜品）；**每个分类一个颜色**（相邻不同色）；
      菜品格子用分类底色 + 文字（去掉图案占位）；弹出窗（个性化定制、选打印机）都**居中显示**
